...
!-------------------------------------- LICENCE END --------------------------------------
     ...
     1                    FICE,FLS,ILG,IL1,IL2,N_step_met)
C=======================================================================
...
C ----* INPUT FIELDS *------------------------------------------------
C
      INTEGER :: ILG,IL1,IL2,N_step_met
      REAL,DIMENSION(ILG) :: VA,T0,TA,QA,PRES,ZREFM,ZREFH,FICE,FLS
C
C ----* OUTPUT FIELDS *------------------------------------------------
C
      REAL,DIMENSION(ILG) ::  CDH, CDM, CDMN
C
...
c         PRINT*, I
        ...
        VSQ=VA(I)*VA(I)
        RESID=999.0
c        PRINT*, RESID,VSQ,VA(I),G,ZREFM(I),Cold,CHARN,
c     2          ZREFM(I)*G/(CHARN*Cold*VSQ)
        DO
...
C        IF(ABS(DENOM).LT.1.0E-6) DENOM=SIGN(1.0E-6,DENOM)
      ...
      IF(ITER.GE.ITMAX)      THEN
        CDM(I)=CDMN(I)
        CDH(I)=CDHN
c        print*, ITER, ITMAX, I,N_step_met
      ENDIF
