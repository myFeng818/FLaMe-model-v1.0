...
!-------------------------------------- LICENCE END --------------------------------------
     ...
     1                   QSWINV, QSWINI, QLWIN, UWIND, VWIND, TA, QA,
     2                   RHOAIR, PADRY, PRES, CSZ, ZREFM, ZREFH,
     3                   ZDIAGM, ZDIAGH, R, TR, S, TS, RHOSNI, RADJ,
     4                   ASVDAT, ASIDAT, FSDB, FSFB, FSSB, REFSN, BCSN,
     5                   ILG, IL1, IL2, JL, NLAKMAX, ISLFD, IZREF, ITG,
     6                   IALS, NBS, ISNOALB, IGL, IRSTRT,
     7                   NSTEP, IYEAR, IDAY, IHOUR, IMIN, TSED,
     8     Clabile,FPP,FRESP,FBUR,CH4,O2,Area,limP,N_step_met,
     1                        FatmO2,Fw,FPPWC,FSAER,
     6      FswDiff,FatmCH4,H_s,Pchl,FsCH4,FsDiff,FsEbul,zebmin,
...
C     * MAY 26/21 - M.MAISONNIER  ADD BIOCHEMICAL MODULE FOR CH4 FLUXES  
C     * 
...
C
     ...
     1                      EXPW,DTEMP,TKE,DELU,GRED,RHOMIX,TSED,ROFICEH
      ...
      REAL, DIMENSION (ILG,NLAKMAX_bgc) :: QFLX_bgc
      REAL,DIMENSION(ILG) :: HLAK_bgc, HDPTH_bgc,ext_C
...
C ----* INTEGER CONSTANTS *-------------------------------------------
C
      INTEGER :: ISNOW,ISLFD,IZREF,ITG,ILG,IL1,IL2,JL,IALS,NBS,ISNOALB,
     1           IGL 
...
C
     ...
     1                         Q0_m,FLGL_m,HFSL_m,HEVL_m,T0_m,
     1                         LKICEH_m,SNO_m,
     2                         RHOSNI_m,S_m,R_m,OSW_m,RHOSNO_m,SNICEH_m
     ...
     1                         FatmCH4_m,Fw_m,FBUR_m
      ...
      REAL, DIMENSION (ILG,NLAKMAX_bgc) :: Clabs_1_m,Clabs_2_m,Clabs_3_m
     ...
     1                                 FsCH4_C_3_m
      ...
      REAL, DIMENSION(ILG,NLAKMAX+1):: K_diff_m
      ...
      REAL, DIMENSION (ILG) :: HDPTH_bgc_m,VOL_phot, VOL_tot
...
C      PRINT*, 'class L 42'
     ...
     3            THLIQ,THLMIN,DELZW,RHOSNO,ZSNOW,ZPONDL,
     8            IWATER,IEVAP,ITERCT,ISAND,                      
     9            ISLFD,ITG,ILG,IGL,IL1,IL2,JL,NBS,ISNOALB,        
     3            TSTEP,TVIRTS,EVBETA,Q0SAT,RESID,
     2            DCFLXM,CFLUXM,WZERO,TRTOP,A,B,
     1            LZZ0,LZZ0T,FM,FH,ITER,NITER,JEVAP,KF)
C
C      PRINT*, 'class L 43'
...
C --UPDATE SEDIMENT TEMPERATURE
     ...
     1          (FFLX(I,NLEV)-FFLXSED+QSED*FSGL(I))
C  
...
C
C-----------------------------------------------------------------------
C          BIOCHEMICAL MODULES (Manon Maisonnier & Aidin Jabbari)
C-----------------------------------------------------------------------
c        print*, 'yes begining'

       CALL geometry_bgc(ILG,I,DELZLK,HLAK,HLAK_bgc,
     ...
     2            LLAK,Area,NSTEP)
       ...
       CALL initialisation(ILG,IL1,IL2,NLAK_bgc,NLAKMAX_bgc,
     ...
     3                          Fsed_1,Fsed_2,Fsed_3,Fibs)
        DO J = 1,NLAK_bgc(I)
          IF ( J .le. 1 ) THEN
            ZPROD(I) = 0.5
          ELSEIF ( exp(-(BLAK(I))*J*DELZLK) > 0.01 ) THEN       !+ext_C(I)         
            ZPROD(I) = J*DELZLK
          ENDIF
        ENDDO
       ZPhotic(I) = ZPROD(I)
...
c       print*, 'yes initial'
       ...
       CALL  Clabile_module(NSTEP,ILG,I,NLAKMAX_bgc,NLAK_bgc,DELZLK,
     1                  TLAK_bgc,Clabile,HDPTH_bgc, Area,
     2                  FPP,FRESP,FBUR,
     3                  IDAY,IHOUR,IMIN,
     5                  LKICEH,limP,QSWIN, DEGLAT, ext_C,ZPROD,
     6                  VOL_phot, VOL_tot )


 
           DO J=1,NLAK_bgc(I)
           IF (isnan(O2(I,J)) .OR. isnan(CH4(I,J)) .OR.
     1           CH4(I,J).gt.HUGE(CH4(I,J)) )  THEN
           PRINT*, 'Clab', NSTEP, O2(I,J), CH4(I,J),OxCH4(I,J)
           STOP
           ENDIF
           ENDDO
...
c       print*, 'yes clab'
       ...
       CALL  CH4_module(NSTEP,ILG,I,NLAKMAX_bgc,NLAK_bgc,DELZLK,
     ...
     5                  LKICEH,limP,
     4     FswDiff,
     6               FatmCH4,H_s,Pchl,FsCH4,FsDiff,FsEbul,zebmin,
     7    Clabs_1,Clabs_2,Clabs_3,FsCH4_C_1,FsCH4_C_2,FsCH4_C_3,
     3     Fsed_1,Fsed_2,Fsed_3, K_diff,DTHERMO , FatmEbul,
     7       Fibs, previous_ice, f_sed ,
...
c       print*, 'yes CH4'
       ...
       CALL O2_module(NSTEP,ILG,I,NLAKMAX_bgc,NLAK_bgc,DELZLK,
     1                TLAK_bgc,O2,Area,HDPTH_bgc,FPP,FRESP,
     4                JMIX,VA,
     5                LKICEH,
     6                     FatmO2,Fw,FPPWC,FSAER,
...
c       print*, 'yes O2'
       ...
       CALL oxidation(NSTEP,ILG,I,NLAKMAX_bgc,NLAK_bgc,O2,
     1					  CH4,TLAK_bgc,OxCH4)
           DO J=1,NLAK_bgc(I)
           IF (isnan(O2(I,J)) .OR. isnan(CH4(I,J)) .OR.
     1           isnan(Clabile(I)) )  THEN
           PRINT*, 'FLaMe', NSTEP, 'O2 = ', O2(I,J), 
     1              'CH4 = ' , CH4(I,J),
     2              'Clabile = ' ,  Clabile(I)
           STOP
           ENDIF
...
C
 ...
 605  CONTINUE
       ...
       CALL Writing(NSTEP,NSTEP_write,ILG, IL1, IL2, JL, 
